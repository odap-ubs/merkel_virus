...
### Folders
cd /mnt/hydra/ubs/shared/users/Sandra/merkel_polyomavirus/res
mkdir trimmed
mkdir alignments
mkdir alignments/unmapped
mkdir virus_align
mkdir virus_align/virus_mapped
...
/share/apps/FastQC/fastqc $FASTQ/C1728ACXX_2_1ss_1.fastq.gz
/share/apps/FastQC/fastqc $FASTQ/C1VTNACXX_4_9ss_1.fastq.gz
...
for file in $TRIMMED/*_1_val_1.fq.gz
do
...
for file in $ALIGNMENTS/*.sorted.bam
do
...
for file in $UNMAPPED/*_unmapped_reads.bam
do
...
$SAMTOOLS view $VIRUS_MAPPED/$base".virus.mapped.bam" | cut -f3 | sort | uniq -c | awk -v file=$base -F'\t' 'BEGIN{OFS="\t"}{print file,$1,$2}' >> $RES/virus.read.counts3.txt
